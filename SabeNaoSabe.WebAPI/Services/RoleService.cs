...
namespace SabeNaoSabe.WebAPI.Services
{
    public class RoleService : IRoleService
    {
        ...
        public async Task<bool> AddUserRolesAsync(string userEmail, string[] roles)
        {
            ...
            var exitRoles = await ExistsRolesAsync(roles);
            if(user != null && exitRoles.Count == roles.Length)
            {
                var assginRoles = await _userManager.AddToRolesAsync(user, exitRoles);
        ...
        {   
            var roleList = _roleManager.Roles.Select(x => new RoleModel { Id =Guid.Parse( x.Id), Name = x.Name }).ToList();
            return roleList;
        }
